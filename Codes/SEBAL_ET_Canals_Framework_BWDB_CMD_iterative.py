...
irrigation_canals= ee.FeatureCollection('users/skhan7/PhD/Chapter2/BWDB_Commad_Area_Simplified_Modified2')  #latest one correct one
...
canal_list = irrigation_canals.reduceColumns(ee.Reducer.toList(1), ['CNLNM_ID']).get('list').getInfo()
      ...
      for canal in canal_list:
        ...
        if os.path.exists(rf"{save_data_loc}/landsat/sebal/" + wktime + r"/sebal_eto_" + regionid + ".zip") ==False:
          try:
            ...
            table = irrigation_canals.filter(ee.Filter.equals('CNLNM_ID', regionn))
...
def ET_Values(penman_mean_values,sebal_mean_values,irr_mean_values):
    ...
    PET_df = pd.DataFrame(formatted_penman_mean_values, columns=['CNLNM_ID', 'PET'])
    SET_df = pd.DataFrame(formatted_sebal_mean_values, columns=['CNLNM_ID', 'SET'])
    Irr_df = pd.DataFrame(formatted_irr_mean_values, columns=['CNLNM_ID', 'Irr'])
    ET_df = PET_df.merge(SET_df, on = 'CNLNM_ID',how = 'outer')
    ET_df = ET_df.merge(Irr_df,  on = 'CNLNM_ID',how = 'outer')
...
def imergprecip():
    ...
    if abs(days_difference) <= 7:
      for dayi in range(1,8):
          ...
          path = r'https://saswe@uw.edu:saswe@uw.edu@jsimpsonhttps.pps.eosdis.nasa.gov/imerg/gis/early/3B-HHR-E.MS.MRG.3IMERG.' + datestr + '-S233000-E235959.1410.V07B.1day.tif'    # Goto link and check if it exist, if error comes
          ...
          r = requests.get(r'https://jsimpsonhttps.pps.eosdis.nasa.gov/imerg/gis/early/3B-HHR-E.MS.MRG.3IMERG.' + datestr + '-S233000-E235959.1410.V07B.1day.tif', auth=('saswe@uw.edu', 'saswe@uw.edu'))  # Goto link and check if it exist, if error comes
          with open(fpath, 'wb') as f:
              f.write(r.content)
          # print('Just wrote',fpath)    
          os.system(f'{osgeo_path} gdalwarp -of GTiff -tr 0.1 0.1 -te 88.5 25.5 89.5 26.5 -overwrite ' + fpath + rf" {save_data_loc}/precip/precip.imerg." + datestr + '.tif')
      ...
      for dayi in range(1,8):
          ...
          if deltime.year == 2024:
              path = rf'https://saswe@uw.edu:saswe@uw.edu@jsimpsonhttps.pps.eosdis.nasa.gov/imerg/gis/early/{deltime.year}/{deltime.month}/3B-HHR-E.MS.MRG.3IMERG.' + datestr + '-S233000-E235959.1410.V06E.1day.tif'  
              r = requests.get('https://jsimpsonhttps.pps.eosdis.nasa.gov/imerg/gis/'+str(deltime.year)+'/'+str(deltime.month).zfill(2)+'/3B-HHR-L.MS.MRG.3IMERG.'+ datestr + '-S233000-E235959.1410.V06C.1day.tif', auth=('saswe@uw.edu', 'saswe@uw.edu'))
          elif deltime.year == 2023:
              path = f'https://saswe@uw.edu:saswe@uw.edu@jsimpsonhttps.pps.eosdis.nasa.gov/imerg/gis/early/'+str(deltime.year)+'/'+str(deltime.month)+'/3B-HHR-E.MS.MRG.3IMERG.' + datestr + '-S233000-E235959.1410.V07B.1day.tif'  
              r = requests.get('https://jsimpsonhttps.pps.eosdis.nasa.gov/imerg/gis/early/'+str(deltime.year)+'/'+str(deltime.month).zfill(2)+'/3B-HHR-E.MS.MRG.3IMERG.'+ datestr + '-S233000-E235959.1410.V07B.1day.tif', auth=('saswe@uw.edu', 'saswe@uw.edu'))
            ...
            path = rf'https://saswe@uw.edu:saswe@uw.edu@jsimpsonhttps.pps.eosdis.nasa.gov/imerg/gis/early/{deltime.year}/{deltime.month}/3B-HHR-E.MS.MRG.3IMERG.' + datestr + '-S233000-E235959.1410.V07B.1day.tif'    # Goto link and check if it exist, if error comes
            r = requests.get('https://jsimpsonhttps.pps.eosdis.nasa.gov/imerg/gis/early/'+str(deltime.year)+'/'+str(deltime.month).zfill(2)+'/3B-HHR-E.MS.MRG.3IMERG.'+ datestr + '-S233000-E235959.1410.V07B.1day.tif', auth=('saswe@uw.edu', 'saswe@uw.edu'))
          # print('Downloaded from: ',path)
          logging.info((yrstr, monthstr, datestr, path))
          fpath = rf"{save_data_loc}/precip/global.precip.imerg." + datestr + '.tif'
          # print('Trying to log into jsimpsonhttps')
        #   r = requests.get(rf'https://saswe@uw.edu:saswe@uw.edu@jsimpsonhttps.pps.eosdis.nasa.gov/imerg/gis/early/{start_date_Script.year}/{start_date_Script.month}/3B-HHR-E.MS.MRG.3IMERG.' + datestr + '-S233000-E235959.1410.V06E.1day.tif', auth=('saswe@uw.edu', 'saswe@uw.edu'))  # Goto link and check if it exist, if error comes
        #   r = requests.get(path, auth=('saswe@uw.edu', 'saswe@uw.edu'))  # Goto link and check if it exist, if error comes
          with open(fpath, 'wb') as f:
              f.write(r.content)
          # print('Just wrote',fpath)    
          os.system(f'{osgeo_path} gdalwarp -of GTiff -tr 0.1 0.1 -te 88.5 25.5 89.5 26.5 -overwrite ' + fpath + rf" {save_data_loc}/precip/precip.imerg." + datestr + '.tif')
...
def union_info():
    ...
    canal_shp_reprojected.set_index('CNLNM_ID', inplace=True)
