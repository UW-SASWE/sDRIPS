...
def sebal_eto_Landsat():
  ...
  with tqdm(total=total_iterations_sebal, desc="Estimating Penman ET and SEBAL ET", unit="Command Area") as pbar:
    for wktime in run_week:
      ...
      enddate = datetime.datetime.strptime(start_date, "%Y-%m-%d") + datetime.timedelta(days = dayVal+2)
      ...
      for canal in canal_list:
        ...
        if os.path.exists(rf"{save_data_loc}/landsat/sebal/" + wktime + r"/sebal_eto_" + regionid + ".zip") ==False:
          try:
            ...
            table = irrigation_canals.filter(ee.Filter.equals('Canal_ID', regionn))
            ...
            # print('Date of the image:',my_datetime)
           ...
           #   print('tempCold',tempCold.getInfo())
            ...
            ETA_24_mask=ETA_24_mask.updateMask(NDVI.gte(0)).unmask(0) #CHANGED THE NDVI  THRESHOLD FOR CLIMATOLOGY FROM 0.2 to 0
            
            ETA_24_mask = ETA_24_mask.updateMask(glcc_crop)
            ETA_mask = ETA_24_mask.multiply(ee.Image.constant(dayVal))
            #Map.addLayer(NDVI,{},'NDVI')
            ETA_mask=ETA_24_mask.clip(ROI).reproject(
              crs= proj,
              scale= selscale
            )
            ...
            avgET= ETA_24_mask.reduceRegion(
              reducer= ee.Reducer.mean(),
              geometry= ROI,
              scale= selscale,
              maxPixels= 1e11
            ...
            logging.error("Week: " + str(wktime) + ",Region: " + str(regionid) +  " Showed Error During Processing.")
            #logging.error(e)
            continue
          pbar.update(1)
...
def makeDirectory():
   ...
   folders_list = ['landsat','uploads','precip','avgt','tmax','tmin','ugrd','vgrd','wind','uploads']
        ...
        for param in ['sebal','penman','irrigation']:
            try:
              ...
              for fn in tqdm(files_to_process, desc=f"Processing {param} for {weeki}", unit="file"):
                    in_fn= r"../Data"+ r'/'+sensor + r'/' + param + r'/' + weeki + r'/' + fn
...
def imergprecip():
    ...
    days_difference = (start_date - today_date).days
    if abs(days_difference) <= 7:
      for dayi in range(1,8):
          ...
          path = r'https://saswe@uw.edu:saswe@uw.edu@jsimpsonhttps.pps.eosdis.nasa.gov/imerg/gis/early/3B-HHR-E.MS.MRG.3IMERG.' + datestr + '-S233000-E235959.1410.V06E.1day.tif'    # Goto link and check if it exist, if error comes
          ...
          r = requests.get(r'https://jsimpsonhttps.pps.eosdis.nasa.gov/imerg/gis/early/3B-HHR-E.MS.MRG.3IMERG.' + datestr + '-S233000-E235959.1410.V06E.1day.tif', auth=('saswe@uw.edu', 'saswe@uw.edu'))  # Goto link and check if it exist, if error comes
      ...
      for dayi in range(1,8):
          ...
          yrstr = deltime.strftime("%Y")
          monthstr = deltime.strftime("%m")
          path = rf"https://saswe@uw.edu:saswe@uw.edu@jsimpsonhttps.pps.eosdis.nasa.gov/imerg/gis/{start_date.year}/{start_date.month}/3B-HHR-L.MS.MRG.3IMERG." + datestr + '-S233000-E235959.1410.V06C.1day.tif'    # Goto link and check if it exist, if error comes
          # print('Downloaded from: ',path)
          logging.info((yrstr, monthstr, datestr, path))
          fpath = rf"{save_data_loc}/precip/global.precip.imerg." + datestr + '.tif'
          # print('Trying to log into jsimpsonhttps')
...
def gfsdata():
    ...
    logging.info("GFS forecast data is being downloading from NCEP NOAA server for the date "+str(startDate))
    logging.info("GFS forecast data is being downloading from NCEP NOAA server ...")
    ...
    try:
        ...
        pass
    x = os.popen(f'{osgeo_path} gdal_translate -of AAIGrid -b 2 ' + fpath + " {save_data_loc}/precip/precip.gfs.nextweek.asc")
    logging.info(x.read())
    try:
    ...
    with tqdm(total=total_iterations, desc="Downloading and Processing GFS Data", unit="file") as pbar:
      for week in weeks:
          ...
          logging.info(x.read())
    logging.critical('Finished GFS Data Download')     
...
def union_info():
    ...
    rasterpath = rf"{save_data_loc}/landsat"  + r'/irrigation/' + run_week[0] + r'/irrigation_' + canal_list[0][0].replace(' ','-') + '.eta.tif'  
    ...
    canal_shp_reprojected.to_csv(f"{save_data_loc}/Landsat_Command_Area_Stats.csv",index = False)
