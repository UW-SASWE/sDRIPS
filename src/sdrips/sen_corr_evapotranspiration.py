...
from sdrips.utils.sensor_utils import (
    download_sensor_data,
    get_available_sensor_dates,
    get_sensor_data_date,
    sensor_data_transformation,
    bias_correction
)
...
def process_cmd_area_sensor_parallel(config_path, main_logger, log_queue, cores):
  yaml = YAML()
  ...
  with tqdm(total=total_iterations, desc="Estimating ET with models", unit=" Command Area") as pbar:
    for wktime in run_week:
      ...
      with ThreadPoolExecutor(
        ...
        futures = [executor.submit(process_single_cmd_area_sensor, args) for args in args_list]
